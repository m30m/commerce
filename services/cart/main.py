...
async def _enrich(product_id: int, qty: int) -> dict:
    product = await downstream.get_json(
        "product", f"{config.PRODUCT_URL}/products/{product_id}"
    )
    return {
...
async def get_cart(uid: int) -> dict:
    rows = await db.fetch(
        "cart_items",
        "SELECT product_id, qty FROM cart_items WHERE user_id = $1 ORDER BY added_at",
        uid,
    )
    items = await asyncio.gather(
        *(_enrich(r["product_id"], r["qty"]) for r in rows)
    )
